...
export default function Pricing() {
  ...
  const startCheckout = async (pkg) => {
    try {
      ...
      const { data } = await api.post("/payments/checkout", { package_id: pkg, origin_url: origin });
    ...
    <PageContainer testid="pricing-page" maxWidth="max-w-4xl">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-[var(--radius)] bg-[hsl(var(--card))]/85 ring-1 ring-[hsl(var(--border))] p-6 [box-shadow:var(--shadow-inner-marble),var(--shadow-elev-1)]" data-testid="pricing-monthly-card">
          ...
          <Button onClick={() => startCheckout("monthly")} disabled={!!busy} data-testid="pricing-subscribe-monthly" className="mt-6 w-full bg-[hsl(var(--secondary))]/80 ring-1 ring-[hsl(var(--border))] uppercase tracking-[0.18em]">
        ...
        <div className="rounded-[var(--radius)] bg-[hsl(var(--brand))]/8 ring-1 ring-[hsl(var(--brand))]/40 p-6 [box-shadow:var(--shadow-inner-marble),var(--shadow-elev-1)] crimson-breathe relative" data-testid="pricing-annual-card">
          ...
          <Button onClick={() => startCheckout("annual")} disabled={!!busy} data-testid="pricing-subscribe-annual" className="mt-6 w-full bg-[hsl(var(--brand))] hover:bg-[hsl(var(--brand))]/90 text-[hsl(var(--brand-foreground))] uppercase tracking-[0.18em]">
