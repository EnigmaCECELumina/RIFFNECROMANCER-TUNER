...
export default function Tuner() {
  ...
  const smoothedPitchRef = useRef({ frequency: 0, clarity: 0 });
  const [displayPitch, setDisplayPitch] = useState({ frequency: 0, clarity: 0 });
  ...
  // Smooth pitch values to prevent flickering
  useEffect(() => {
    if (!active) {
      smoothedPitchRef.current = { frequency: 0, clarity: 0 };
      setDisplayPitch({ frequency: 0, clarity: 0 });
      return;
    }

    const smoothingFactor = 0.2;
    const minClarityThreshold = 0.85;

    if (pitch.clarity > minClarityThreshold && pitch.frequency > 60) {
      if (smoothedPitchRef.current.frequency === 0) {
        smoothedPitchRef.current = { ...pitch };
      } else {
        smoothedPitchRef.current = {
          frequency: smoothingFactor * pitch.frequency + (1 - smoothingFactor) * smoothedPitchRef.current.frequency,
          clarity: smoothingFactor * pitch.clarity + (1 - smoothingFactor) * smoothedPitchRef.current.clarity,
        };
      }
      setDisplayPitch({ ...smoothedPitchRef.current });
    } else {
      // Decay when signal is weak
      smoothedPitchRef.current = {
        frequency: smoothedPitchRef.current.frequency * 0.95,
        clarity: smoothedPitchRef.current.clarity * 0.95,
      };
      if (smoothedPitchRef.current.frequency < 1) {
        smoothedPitchRef.current = { frequency: 0, clarity: 0 };
      }
      setDisplayPitch({ ...smoothedPitchRef.current });
    }
  ...
  const note = frequencyToNote(displayPitch.frequency || 0);
  const nearest = nearestDropDString(displayPitch.frequency || 0);
        ...
        <div className="flex items-center justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex items-baseline gap-3">
              ...
              <span className="text-[hsl(var(--text-2))] text-sm tabular-nums" data-testid="tuner-frequency-readout">{displayPitch.frequency ? displayPitch.frequency.toFixed(1) : "0.0"} Hz</span>
            </div>
          </div>
